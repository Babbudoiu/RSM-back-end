...
exports.createBooking = async (req,res) => {
    try {
        const customer = new Bookings({day:req.body.day,dayArray:req.body.dayArray});
...
exports.updateReservation = async (req, res) => {
    try {
       ...
       const modifyBookings = await Bookings.findOneAndUpdate({day:req.body.day},{dayArray:req.body.dayArray});
       res.status(200).send({ user: modifyBookings, message: "Bookings Modified"}) 
    } catch (error) {
        res.status(500).send(error) 
    }
};
