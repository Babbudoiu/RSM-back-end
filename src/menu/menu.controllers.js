...
exports.createItem = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ menu: savedItem, message: "Menu item created in database" });
  } catch (error) {
...
exports.deleteItem = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ name: removeItem, message: "Menu item deleted from database" });
  } catch (error) {
    res.status(500).send(error);
  }
};
...
exports.updateItem = async (req, res) => {
  try {
    ...
    const modifyItem = await Menu.updateOne({
      name: name,
      price: price,
      course: course,
    });
